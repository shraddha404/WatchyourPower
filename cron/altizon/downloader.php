...
foreach($sensors as $sensor){
	if(array_key_exists('Prayas-ID',$sensor['properties'])){
		if($sensor['properties']['Prayas-ID'] !=''){
			$device_details [$i]['sensor']=$sensor['thing_key'];
			$device_details [$i]['device_id']=$sensor['properties']['Prayas-ID'];
			$device_details [$i]['sim_id']=$sensor['properties']['Sim-ID'];
			$device_details [$i]['lat']=$sensor['properties']['Lat'];
			$device_details [$i]['long']=$sensor['properties']['Long'];
			$device_details [$i]['fw_ver']=$sensor['properties']['FW Ver'];
			$i++;
		}
	}
}
...
//	$events = download_raw_events($ak, $device_details[2]['sensor'], $argv[1], $argv[2], $api_host);
    ...
    foreach ($device_details as $sk) {
	  ...
	  if(count($events) == 0){
		 // if all 24 events are missing for a device,
		 // log one entry in the altizon_downloads table
		 // hr should be null here
		$missing_hr[$x]['device_id']=$sk['device_id'];
		$missing_hr[$x]['date']=date('Y-m-d',strtotime($from_date));
		$missing_hr[$x]['hr']= null; // instead of the hr value
		...
		echo "No data for Device ID ".$sk['device_id']." for the day ".$missing_hr[$x]['date'].". Will be marked as missing-day.\n";
		$x++;
	 	continue;
	  }
	   $pre_hr=0; 	
	 	...
	 	$sdate= $eve[2];
	   	if($i%60 == 0 && empty($events[($i+59)])){
			...
			for($m_h = $first_missed_hr; $m_h<24; $m_h++){
				...
				$missing_hr[$x]['sim_id']=$sk['sim_id'];
				$missing_hr[$x]['fw_ver']=$sk['fw_ver'];
				echo "Incomplete or no hourly data of hr ".$m_h. " of ".$missing_hr[$x]['date']." for Device ID ".$sk['device_id'].". Will be marked as missing hr.\n";
				$x++;
			}
			break;
		}
